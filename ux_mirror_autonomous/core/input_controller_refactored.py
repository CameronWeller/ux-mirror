#!/usr/bin/env python3
"""
Refactored Autonomous Input Controller for UX-MIRROR Testing
Demonstrates Improvement #1: Safety Check Decorator Pattern

This refactored version shows how the decorator pattern simplifies the code
and enables new features like performance monitoring and custom safety policies.
"""
...
try:
    ...
    print("Run: pip install pyautogui psutil")
    raise

# Import our new decorators
from .decorators import safety_check, retry, screenshot_on_error, validate_bounds, thread_safe
...
class AutonomousInputController:
    """
    Refactored input controller demonstrating decorator pattern usage.
    
    Key improvements:
    1. Removed repetitive safety checks - now handled by @safety_check decorator
    2. Added automatic performance monitoring
    3. Enabled custom pre/post conditions per method
    4. Added retry capabilities for network operations
    5. Automatic screenshot on errors
    6. Thread-safe operations where needed
    """
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("AutonomousInputController initialized (refactored version)")
    ...
    @safety_check(
        ...
        log_calls=False  # Reduce noise for frequent operations
    )
    ...
    def move_mouse(self, x: int, y: int, duration: float = 1.0, 
        ...
        Now with:
        - Automatic safety checking via decorator
    ...
    def click(self, x: Optional[int] = None, y: Optional[int] = None, 
        ...
        Simplified with decorator pattern - no manual safety checks needed.
        """
        try:
    ...
    @safety_check()
    @validate_bounds(x_param='start_x', y_param='start_y')
    @validate_bounds(x_param='end_x', y_param='end_y')
    def drag(self, start_x: int, start_y: int, end_x: int, end_y: int, 
             duration: float = 1.0, button: str = 'left') -> bool:
        """Drag from start to end position with enhanced safety and validation"""
        try:
    ...
    def wait_for_element(self, image_path: str, timeout: float = 10.0, 
        ...
        Now with retry capability for better reliability.
        """
        try:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        """
        Get performance metrics for all decorated methods.
        
        This is a new feature enabled by the decorator pattern!
        """
        from .decorators import performance_metrics
        
        metrics = {}
        for method_name in ['move_mouse', 'click', 'type_text', 'key_press', 
                           'scroll', 'drag', 'wait_for_element']:
            full_name = f"AutonomousInputController.{method_name}"
            stats = performance_metrics.get_stats(full_name)
            if stats:
                metrics[method_name] = stats
        
        return metrics

def main():
    """Test the refactored input controller"""
    print("🤖 Testing Refactored Autonomous Input Controller...")
    ...
    for method, stats in metrics.items():
        if stats:
            print(f"  {method}: {stats['count']} calls, avg {stats['avg_time']:.3f}s")
    
    controller.end_session("test_session")
    
    print("✅ Refactored input controller test completed!")
